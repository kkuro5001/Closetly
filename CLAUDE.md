...
Pipeline: `Flutter (frontend) → Go/Gin (Go) → Python/FastAPI + YOLOv8 (Python)`, with image storage in Supabase Storage and clothing metadata in a local SQLite database (no Supabase Postgres tables are used — auth and storage are the only Supabase services in play).
...
4. `camera_page.dart` merges the AI result (category/color/suggestion) with user-edited fields and calls `DatabaseHelper.instance.insertClothing`, storing the Supabase Storage *path* (not a signed URL) in the local SQLite `clothes` table.
5. `frontend/lib/pages/closet_page.dart` reads clothing rows from SQLite and resolves each stored path to a fresh signed URL via `storageService.getSignedUrl` at display time (the bucket is private, so URLs are not persisted — they're re-signed on every read).
...
### Local database
`frontend/lib/database/database_helper.dart` wraps `sqflite`. Schema is a single `clothes` table (`id`, `imagePath` — a Supabase Storage path, `category`, `color`, `season`); see `frontend/lib/models/clothing.dart` for the corresponding model/mapping. This is mobile/desktop-only; it is not initialized for Flutter Web.

### Supabase (Storage)
Bucket: `closet-images` (private). Objects are namespaced as `{userId}/original/{fileName}` and `{userId}/processed/{fileName}` — the RLS policies below rely on the first path segment being the uploader's `auth.uid()`.

```sql
-- Bucket (private; URLs are only ever obtained as short-lived signed URLs)
